...
public class ContaCorrente extends Conta {
	...
	public ContaCorrente(int numero, Pessoa titular, double saldo, Agencia agencia) {
		...
		this.tipo = TipoContaEnum.CONTA_CORRENTE;
	}
	...
	public void transferir(Conta contaDestino, double valor) {
		...
		if (valor > 0 && this.saldo >= valor + TAXA_TRANSFERENCIA) {
			...
			try {
				escreverDados.listaTransacoes(contaDestino);
	...
	public void tirarRelatorioTaxa() {
		System.out.println("Taxas de saque: R$" + String.format("%.2f", this.totalTaxaSaque));
		System.out.println("Taxas de depósito: R$" + String.format("%.2f", this.totalTaxaDeposito));
		System.out.println("Taxas de transferência: R$" + String.format("%.2f", this.totalTaxaTransferencia));
		System.out.println("Soma das taxas: R$"+ String.format("%.2f", (this.totalTaxaSaque + this.totalTaxaDeposito + this.totalTaxaTransferencia)));
		
		if(this.possuiSeguro == true) {
			System.out.println("Valor Seguro de Vida: R$" + this.valorSeguro);
		}
	}
	...
	public String tirarRelatorioTaxasArquivo() {
		...
		return "Taxas de saque: R$" + String.format("%.2f", this.totalTaxaSaque) + 
 		  "\n" + "Taxas de depósito: R$" + String.format("%.2f", this.totalTaxaDeposito) +
 		  "\n" + "Taxas de transferência: R$" + String.format("%.2f", this.totalTaxaTransferencia) +
 		  "\n" +  "Soma das taxas: R$" + String.format("%.2f", (this.totalTaxaSaque + this.totalTaxaDeposito + this.totalTaxaTransferencia))+
 		  "\n" + textoSeguro;
